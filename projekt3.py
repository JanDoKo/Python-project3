...
def check_command_values(url,soup):
    os.system('cls')
...
def voters_data(Description, url_list, id) -> list:
    '''
    Do funkce budou zadány hodnoty
     - description - pouze pro popis uživateli, jaká hodnota je právě zpracovávána
...
def parties_results(Description, url_list) -> list:
    '''
    Do funkce budou zadány hodnoty
     - description - pouze pro popis uživateli, jaká hodnota je právě zpracovávána
     - url_list - list adres, které budou zpracovány v procesu
...
def make_final_tuple() -> tuple:
    '''
    Zpracování všech záznamů (listů) do jednoho tuplu
    '''
    url_list = support_url()
    result = parties_results('výsledky stran', url_list)
    final = (result)
    valid = voters_data('počty platných hlasů', url_list, 'sa6')
    for x in range(len(valid)):
        final[x].insert(0,valid[x])
    envelope = voters_data('počty odevzadných obálek', url_list, 'sa3')
    for x in range(len(envelope)):
        final[x].insert(0,envelope[x])
    registered = voters_data('počty registrovaných voličů', url_list, 'sa2')
...
def csv_maker(file) -> None:
    new_csv = open(file, mode="w", encoding='utf-8', newline='')
    writer = csv.writer(new_csv, delimiter=";")
    writer.writerows(make_final_tuple())
    print(f'Výsledný csv soubor {file} byl vytvoren')
    new_csv.close()
